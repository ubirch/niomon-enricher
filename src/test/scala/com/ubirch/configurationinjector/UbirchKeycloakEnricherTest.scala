package com.ubirch.configurationinjector

import java.util.UUID

import com.typesafe.config.{ConfigFactory, ConfigValueFactory}
import com.ubirch.kafka.MessageEnvelope
import com.ubirch.niomon.base.{NioMicroservice, NioMicroserviceLive}
import com.ubirch.protocol.ProtocolMessage
import org.apache.kafka.clients.consumer.ConsumerRecord
import org.scalatest.{FlatSpec, Matchers}

class UbirchKeycloakEnricherTest extends FlatSpec with Matchers {
  val config = ConfigFactory.load().withValue(
    "niomon-enricher.ubirch-device-info-url",
    ConfigValueFactory.fromAnyRef("https://api.console.dev.ubirch.com/ubirch-web-ui/api/v1/auth/simpleDeviceInfo")
  )
  // this is ignored by default because it requires having a fresh token gotten from the ubirch keycloak system
  "UbirchKeycloakEnricher" should "inject data from ubirch thing api to ubirch packet envelopes" ignore {

    val microserviceRedisCacheDonor = NioMicroserviceLive[String, String]("niomon-enricher", _ => ???)
    val redisCache = microserviceRedisCacheDonor.redisCache
    // when manually running this test, be sure to replace this with a fresh token for 1b4bc7bf-d333-4fc5-97c7-93c16fc18970
    val token = "eyJhbGciOiJFUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICJfYVJxT3lmTkxyakNwY0x1dmxRWjFIWTJTNl9vMmVRQkl5UjFnTEZJVW80In0..MEUCIQCfqwWNHZuZKhgvnqESb-9ApRhOy9UeuiaFynrHTT2rYAIgZE9eFUV7sKfrFoHKM2-sff2ot_D56cdr7btCTdyYatY"

    val record = new ConsumerRecord[String, MessageEnvelope]("foo", 0, 0, "bar",
      MessageEnvelope(new ProtocolMessage(28, UUID.fromString("1b4bc7bf-d333-4fc5-97c7-93c16fc18970"), 0, null)))
      .withExtraHeaders("X-Ubirch-DeviceInfo-Token" -> token)

    val enricher = new UbirchKeycloakEnricher(new NioMicroservice.Context(redisCache, config.getConfig("niomon-enricher")))

    val newRecord = enricher.enrich(record)

    println(newRecord)
  }
}
